...
//  CryptoTrack
//
//  Created by Furkan Doğan on 2.02.2024.
//
...
class HomeViewModel: ObservableObject {
    ...
    func addSubscribers() {
        ...
        $allCoins
            ...
            .sink { [weak self] (returnedCoins) in
                ...
                self.portfolioCoins = sortPorfolioCoinsIfNeeded(coins: returnedCoins)
            }
            .store(in: &cancellables)
    ...
    private func filterAndSortOption(text: String, coins: [Coin], sortOption: SortOption) -> [Coin] {
    ...
    private func sortPorfolioCoinsIfNeeded(coins: [Coin]) -> [Coin] {
        switch sortOption {
        case .holdings:
    ...
    private func mapGlobalData(marketData: MarketData?, portfolioCoins: [Coin]) -> [Statistic] {
        var stats = [Statistic]()
